...
export class RedemptionsRepository {
  ...
  async findPublishedOfferById(offerId: string): Promise<Offer | null> {
    return this.offersRepo.findOne({ where: { id: offerId, status: OfferStatus.PUBLISHED } });
  }
  ...
  async countUserOfferUsages(userId: string, offerId: string): Promise<number> {
    return this.redemptionsRepo.count({
      where: {
        userId,
        offerId,
        status: In([RedemptionStatus.CREATED, RedemptionStatus.CONFIRMED, RedemptionStatus.USED]),
      },
    });
  }
  ...
  async isOfferLocationAllowed(offerId: string, locationId: string): Promise<boolean> {
    const count = await this.offerLocationsRepo.count({ where: { offerId, locationId } });
    return count > 0;
  }
  ...
  async listUserRedemptions(userId: string, limit: number, offset: number): Promise<[Redemption[], number]> {
  ...
  async findUserRedemptionById(userId: string, redemptionId: string): Promise<Redemption | null> {
    return this.redemptionsRepo.findOne({ where: { id: redemptionId, userId } });
  }

  async getOffersMap(offerIds: string[]): Promise<Map<string, Offer>> {
    if (offerIds.length === 0) return new Map<string, Offer>();
    const offers = await this.offersRepo.find({ where: { id: In(offerIds) } });
    return new Map(offers.map((offer) => [offer.id, offer]));
  }

  async getPartnerLocationsMap(locationIds: string[]): Promise<Map<string, PartnerLocation>> {
    if (locationIds.length === 0) return new Map<string, PartnerLocation>();
    const locations = await this.partnerLocationsRepo.find({ where: { id: In(locationIds) } });
