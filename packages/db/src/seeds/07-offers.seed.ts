...
const COFFEE_LAB_15_COVER_URL =
  "https://placehold.co/960x540/png?text=Coffee+Lab+15%25";

const COFFEE_LAB_BREAKFAST_COVER_URL =
  "https://placehold.co/960x540/png?text=Breakfast+20%25";
...
export async function seedOffers(dataSource: DataSource): Promise<void> {
  ...
  const partner = await partnerRepo.findOne({
    where: { brandName: "Coffee Lab" },
  });
  const owner = await userRepo.findOne({
    where: { email: "owner@coffeelab.local" },
  });
  const foodCategory = await categoryRepo.findOne({
    where: { slug: "food" },
  });
  ...
  const offer1 = await upsertByWhere(
    offerRepo,
    { slug: "coffee-lab-15" },
    {
  ...
  if (megaLocation) {
    await upsertByWhere(
      offerLocationRepo,
      { offerId: offer1.id, locationId: megaLocation.id },
      {
        offerId: offer1.id,
    ...
    await upsertByWhere(
      offerLocationRepo,
      { offerId: offer2.id, locationId: megaLocation.id },
      {
        offerId: offer2.id,
        locationId: megaLocation.id,
      }
  ...
  if (satbayevLocation) {
    await upsertByWhere(
      offerLocationRepo,
      { offerId: offer1.id, locationId: satbayevLocation.id },
      {
        offerId: offer1.id,
    ...
    await upsertByWhere(
      offerLocationRepo,
      { offerId: offer2.id, locationId: satbayevLocation.id },
      {
        offerId: offer2.id,
        locationId: satbayevLocation.id,
      }
