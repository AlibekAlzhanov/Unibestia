...
const COFFEE_LAB_LOGO_URL = "https://placehold.co/160x160/png?text=CL";
...
export async function seedPartners(dataSource: DataSource): Promise<void> {
  ...
  const partner = await upsertByWhere(
    partnerRepo,
  ...
  await upsertByWhere(
    memberRepo,
    { partnerId: partner.id, userId: owner.id },
    {
  ...
  await upsertByWhere(
    memberRepo,
    { partnerId: partner.id, userId: cashier.id },
    {
      partnerId: partner.id,
      userId: cashier.id,
  ...
  await upsertByWhere(
    locationRepo,
    { partnerId: partner.id, name: "Coffee Lab Mega" },
    {
      partnerId: partner.id,
  ...
  await upsertByWhere(
    locationRepo,
    { partnerId: partner.id, name: "Coffee Lab Satbayev" },
    {
      partnerId: partner.id,
