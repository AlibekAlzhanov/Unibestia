...
function statusClass(status?: string | null): string {
  if (status === "published" || status === "used" || status === "confirmed") {
...
function LoadingPartnerDashboard(): JSX.Element {
  return (
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1fr_360px] lg:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-base leading-8 text-[#DDE8EA]">
              Управляйте скидками, точками продаж, сотрудниками и QR-активациями
              партнёра. Данные берутся из PostgreSQL через tRPC.
            </p>
          ...
          <section className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-start">
                ...
                <div className="mt-5 grid gap-3">
                  {(data?.recentRedemptions ?? []).length === 0 ? (
                    <div className="rounded-[26px] bg-[#F9FAF8] p-6 text-sm leading-7 text-[#6B7280]">
                      Использований пока нет. Когда студенты начнут показывать
                      QR-коды, история появится здесь.
                    </div>
                  ) : (
            ...
            <aside className="space-y-6 lg:sticky lg:top-[92px]">
              ...
              <section className="ub-card rounded-[34px] p-6 md:p-7">
                ...
                <div className="mt-5 grid gap-3">
                  <div className="flex items-center justify-between gap-4 rounded-[22px] bg-[#F9FAF8] px-4 py-3">
                    <span className="text-sm font-bold text-[#526470]">
                      Бренд
                    </span>
                    <span className="truncate text-sm font-black text-[#17384B]">
